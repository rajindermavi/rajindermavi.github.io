...
the population is partitioned into N clusters. 
Interactions are permitted within clusters but not between clusters.
...
The goal is to measure quantities like (@eq-ate) by individual and by group.
They differentiate a number of different causal effects
* Direct - $CE^D_{(i)}$ - The impact on $Y_i$ of altering a single individuals assignment $z_i=0 \to z_i=1$ while holding all other assigments fixed.
...
As an example of the results, defining  $\hat Y_{(i)}(z;\psi)$ to be the average measurement among group $i$ members assigned $Z=z$,
$$
\hat{CE}^D_{(i)} = \hat Y_i(0;\psi) - \hat Y_i(1;\psi) 
$$
