...
The variance of tha average estimator is
$$
Var(\hat \tau_{HT}) 
    = 
...
Sample $n$ units from $\mathcal{K}$ by Simple Random Sampling Without Replacement.
...
$$
Making a simple assumption $n \propto N^\alpha$ 
for $0 < \alpha < 1$ or $n = \beta N$ for $0 < \beta < 1$,
the variance of the estimator of the average tends to zero.